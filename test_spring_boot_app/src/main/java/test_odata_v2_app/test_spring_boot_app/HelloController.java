package test_odata_v2_app.test_spring_boot_app;
...
public class HelloController {
  ...
  private Connection getConnection() {
    ...
    try {
        ...
        String DB_CONN_URL = arr.getJSONObject(0).getJSONObject("credentials").getString("url");
        
        System.out.println("Connection Url " + DB_CONN_URL);
        System.out.println("Username " + DB_USERNAME);
        System.out.println("Password " + DB_PASSWORD);

        conn = (Connection) DriverManager.getConnection(DB_CONN_URL, DB_USERNAME, DB_PASSWORD);
    } catch (Exception e) {
